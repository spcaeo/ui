/**
 * Behavioural tests for folder-tabs, run against demo.html by
 * tools/smoke-test.mjs. Every assertion here guards a bug this control has
 * actually shipped.
 */
export default async function run({ page, check }) {
  console.log("\nvisible panel is never lost");
  {
    ...
    // panel and left the rail floating above nothing — the mechanic gone.
  ...
  console.log("\nARIA wiring");
  {
    ...
    const selectedCounts = await page.evaluate(() =>
      [...document.querySelectorAll('[role="tablist"]')].map(
        (list) =>
          [...list.querySelectorAll('.fldr-scroll > [role="tab"]')].filter(
    ...
    check(
      ...
      selectedCounts.every((n) => n === 1),
      JSON.stringify(selectedCounts),
    );

    const roving = await page.evaluate(() =>
      [...document.querySelectorAll('[role="tablist"]')].map(
        (list) =>
          [...list.querySelectorAll('.fldr-scroll > [role="tab"]')].filter((t) => t.tabIndex === 0)
            .length,
      ),
    );
    check(
  ...
  console.log("\nkeyboard");
  {
    const text = () => page.evaluate(() => document.activeElement.textContent.trim());
    await page.locator("section").first().locator(".fldr-tab").first().focus();

    await page.keyboard.press("ArrowRight");
    await page.waitForTimeout(60);
    check("ArrowRight selects the next tab", (await text()) === "Dates");

    // "Preview" is aria-disabled and must be stepped over, not landed on.
    await page.keyboard.press("ArrowRight");
    await page.waitForTimeout(60);
    const afterSkip = await text();
    check(
    ...
    await page.waitForTimeout(60);
    check("Home goes to the first tab", (await text()).startsWith("Rules"));

    await page.keyboard.press("End");
    await page.waitForTimeout(60);
    check("End goes to the last tab", (await text()) === "Jurisdiction");
  }
